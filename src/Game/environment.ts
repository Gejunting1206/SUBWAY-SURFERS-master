...
export const roadLength = 330; // 道路长度
const threeRoad = [-roadWidth / 3, 0, roadWidth / 3]; // 三条道路的 x 坐标
const [house1SceneX, house1SceneY, house1RightRotation] = [roadWidth * 1.9, 0, Math.PI]; // 房子1的 x 坐标、y 坐标、右侧旋转角度
const [house2SceneX, house2SceneY, house2RightRotation] = [roadWidth * 1.6, 0, Math.PI]; // 房子2的 x 坐标、y 坐标、右侧旋转角度
const [house3SceneX, house3SceneY, house3RightRotation] = [roadWidth * 1.08, 10, Math.PI / 2]; // 房子3的 x 坐标、y 坐标、右侧旋转角度
const [house4SceneX, house4SceneY, house4RightRotation] = [roadWidth * 1.5, 0, Math.PI]; // 房子4的 x 坐标、y 坐标、右侧旋转角度
const [house5SceneX, house5SceneY, house5RightRotation] = [roadWidth * 1.2, 0, Math.PI]; // 房子5的 x 坐标、y 坐标、右侧旋转角度
...
export default class Environment {
    ...
    planeGroup: THREE.Group = new THREE.Group(); // 地面组
    plane: THREE.Object3D[] = []; // 地面对象数组
    obstacal: THREE.Object3D[] = []; // 障碍物对象数组
    coin: THREE.Object3D[] = [];  // 金币对象数组
    z!: number; // 场景的 z 坐标
    house1Scene: any; // 房子1场景
    house2Scene: any; // 房子2场景
    ...
    startGame() {
        ...
        this.z = -1 * (roadLength / 2) + 10;
        this.coin = [];
        this.setAmbientLight(); // 设置环境光
        this.setGroupScene(this.z, -5, true); // 设置场景组
    }
    ...
    async setPlane(modelGroup: THREE.Group, z: number) {
        ...
        const [planeTexure, planeTexure1] = await Promise.all([
            await textureload('/assets/png/railway_texture.png'), // 加载铁路纹理
            await textureload('/assets/png/stone.png'), // 加载石头纹理
        ]);
    ...
    async loadmodelAndSize(modelGroup: THREE.Group, houseZ: number, load: boolean) {
        if (load) {
            const [
                ...
                await load3DModel('/assets/glb/house1.glb'), // 加载房子1模型
                await load3DModel('/assets/glb/house2.glb'), // 加载房子2模型
                await load3DModel('/assets/glb/house3.glb'), // 加载房子3模型
                await load3DModel('/assets/glb/house4.glb'), // 加载房子4模型
                await load3DModel('/assets/glb/house5.glb'), // 加载房子5模型
            ]);
